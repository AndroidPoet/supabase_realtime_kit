...
/// How much we trust a peer's public key.
enum TrustLevel {
  /// We have never seen this user's public key.
  unknown,
...
/// A recorded identity for one peer: the public key plus its [TrustLevel].
class TrustEntry {
  /// Creates an entry.
  const TrustEntry({required this.publicKey, required this.level});

  /// The peer's X25519 public key (32 bytes).
  final Uint8List publicKey;

  /// How much we trust [publicKey].
...
/// Persists the per-peer identity ledger: which public key we have seen for
